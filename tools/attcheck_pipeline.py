...
def main(mymodel, file_test):
    ...
    #filename_base, ext = os.path.splitext(file_test)
    #file_normalized = filename_base+'_normalized'+ext
    #df_test.to_csv(file_normalized, index=False, encoding='utf_8_sig')

    # Pands -> Daasets
    dataset_test=Dataset.from_pandas(df_test)

    #class_labels = ['Non_Risk','Low_Risk','Mid_Risk','High_Risk']
    ...
    for i,example in enumerate(dataset_test):
        ...
        if 'label' in df_test:
            #gtruth_id = df_test.at[i,'label']
            #gtruth_name = class_labels[gtruth_id]
            gtruth_name = df_test.at[i,'label']
            #print(f'gtruth:{gtruth_name}\tpred_label:{predict_label}\tscore:{score}')
