...
def main(model_dir, f_train,f_valid,f_pred):
    ...
    # reading dataset from Hugging Face Hub
    ...
    # From file => datasets.DatasetDictクラスになる (train_dataset['train']でアクセスできる
    #train_dataset = load_dataset('csv', data_files={'train': f_train})
    #valid_dataset = load_dataset('csv', data_files={'validation': f_valid})
    #print(train_dataset['train'])

    #dtype_dic = {"shop_id": "string", "item_id": "string", "inventory_id": "string", "sentence1": "string", "sentense2" : "string", "label": "string"}
    dtype_dic = {"sentence1": "string", "sentense2" : "string", "label": "string"}
    
    # Pandas経由 -> Datasetクラスになる（ひとまずこっちにする）
    #train_df = pd.read_csv(f_train,dtype='unicode')
    #train_df = pd.read_csv(f_train,low_memory=False)
    train_df = pd.read_csv(f_train,low_memory=False,dtype=dtype_dic)
    #train_df = pd.read_csv(f_train,dtype=dtype_dic)
    train_dataset = Dataset.from_pandas(train_df)
    
    #valid_df = pd.read_csv(f_valid,dtype='unicode')
    #valid_df = pd.read_csv(f_valid,low_memory=False)
    valid_df = pd.read_csv(f_valid,low_memory=False,dtype=dtype_dic)
    valid_dataset = Dataset.from_pandas(valid_df)
    ##class_label = datasets.ClassLabel(num_classes=3, names=['entailment', 'contradiction', 'neutral'])
    ...
    train_dataset = train_dataset.cast_column("label", datasets.ClassLabel(names=['Correct', 'Incorrect']))
    #exit()
    valid_dataset = valid_dataset.cast_column("label", class_label)

    # label information of the dataset
    print('----- dataset features: -----')
    print(train_dataset.features)    
    print('----- dataset class label -----')
    ...
    #model = torch.nn.DataParallel(model) #### <-- うまくいかないが、すでに複数のGPUを利用できている
    ...
    training_args = TrainingArguments(
        output_dir="output_dump",  # 結果の保存フォルダ
        save_total_limit=2,
        ...
        ##evaluation_strategy="epoch",  # 検証セットによる評価のタイミング #04/21 comment out
        eval_strategy="epoch",  # 検証セットによる評価のタイミング #04/21 comment out        
    ...
    #model.module.save_pretrained(model_dir) <- Dataparallelするときに使うらしい:https://github.com/huggingface/transformers/issues/16971
    ...
    pprint('----- my label (class labels:valid_df[label]) -----')        
    print(mylabels)
    print('----- Validation set ------')
    print(classification_report(valid_df['label'], valid_df['pred'], digits=3, target_names=['Correct', 'Incorrect']))
    #print(classification_report(valid_df['label'], valid_df['pred'], digits=3, target_names=['entailment', 'contradiction', 'neutral']))    
    ...
    # Test setでモデルを評価    
    #df_test = pd.read_csv(f_test,dtype='unicode')
    #df_test = pd.read_csv(f_test,low_memory=False)
    df_test = pd.read_csv(f_test,low_memory=False,dtype=dtype_dic)
    ...
    for row in df_test.itertuples():
        ...
        #gtruth = row.label
        #gtruth_label = classlabels[int(gtruth)]
        encoded_input = tokenizer(sentence1, sentence2, max_length=max_len,return_tensors='pt',truncation=True)
        model.eval()
        ...
        with torch.no_grad():
            ...
            #df_append = pd.DataFrame({'gtruth':gtruth_label,'sentence1':sentence,'sentence2':sentence2,'label':classlabels[int(torch.argmax(prediction))],'pred':np.max(prediction.cpu().detach().numpy())},index=[0])
    ...
    if df_gtruth.empty == True:
        ...
        for row in df_gtruth.itertuples():
            ...
            #df_gtruth['label_name'][index] = label_name
...
def eval_model(trainer,test_dataset):
    ...
    print(classification_report(test_df['labels'], test_df['pred'], target_names=['Correct','Incorrect'], digits=3))    
...
#        ) -> BatchEncoding:
    ...
    #encoded_example = tokenizer(example["sentence1"], example["sentence2"], max_length=max_len).to(device)
